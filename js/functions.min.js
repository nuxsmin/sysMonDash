...
return b.html()},dummy:function(){var a=c(".backendDummy").length,b=c(".dummyTemplate").clone();b.find("[name='backend[dummy][alias]']")[0].name="backend[dummy]["+a+"][alias]";b.find("[name='backend[dummy][path]']")[0].name="backend[dummy]["+a+"][path]";b.find("[name='backend[dummy][image]']")[0].name="backend[dummy]["+a+"][image]";b.find("[name='backend[dummy][active]']")[0].name="backend[dummy]["+a+"][active]";return b.html()}},check:{zabbix:function(a){var b=a.parent().parent(".backendConfig");
a=b.find(".backend_zabbix_version").val();var c=b.find(".backend_zabbix_url").val(),d=b.find(".backend_zabbix_user").val(),b=b.find(".backend_zabbix_pass").val();""===a||""===c?alertify.alert(e.getLang(4)):(a={action:"zabbixBackend",data:JSON.stringify({url:c,version:a,user:d,pass:b})},u(a))},smd:function(a){var b=a.parent().parent(".backendConfig");a=b.find(".backend_smd_url").val();b=b.find(".backend_smd_token").val();""===a?alertify.alert(e.getLang(4)):(a={action:"smdBackend",data:JSON.stringify({url:a,
action:10,token:b})},u(a))}}},x=function(){if(e.getAudioEnabled()){var a=document.getElementById("audio-alarm"),b=e.getTimeout()/1E3;a.duration>=b&&!1===h&&a.addEventListener("timeupdate",function(){h=!0;a.currentTime>=b-2&&(a.pause(),a.currentTime=0)});a.play()}},u=function(a){c.ajax({url:m()+"/ajax/checkConfig.php",type:"post",dataType:"json",data:a,success:function(b){b=void 0===b.status||0!==b.status?e.getLang(3)+"<br>"+e.getLang(2)+" "+b.description||"":e.getLang(1)+"<br>"+e.getLang(2)+" "+b.description;
alertify.alert(b)}})};return{startSMD:function(){c.ajaxSetup({global:!1,timeout:e.getTimeout()/2});t();setInterval(function(){t()},e.getTimeout())},getUpdates:function(){c("#updates").load(m()+"/ajax/getUpdates.php")},setConfigHooks:function(){var a=c("#frmConfig");a.on("submit",function(b){b.preventDefault();y(a)}).find("select").each(function(){var b=c(this);b.val(b.data("selected"))});a.on("change","input",function(b){b=c("#warn-save");"block"!==b.css("display")&&b.show()});c(".add-backend").on("click",
function(b){b.preventDefault();switch(c(this).data("type")){case "status":g.add.status();break;case "livestatus":g.add.livestatus();break;case "checkmk":g.add.checkmk();break;case "zabbix":g.add.zabbix();break;case "smd":g.add.smd();break;case "dummy":g.add.dummy()}});c("#backends-config-container").on("click",".backendDelete",function(b){b.preventDefault();if(!0===window.confirm(e.getLang(0))){var a=c(this).parent().parent(".backendConfig");a.slideUp("slow",function(){a.remove()});z()}}).on("click",
".backendCheckSMD",function(b){b.preventDefault();g.check.smd(c(this))}).on("click",".backendCheckZabbix",function(b){b.preventDefault();g.check.zabbix(c(this))});c(".btn-gen-token, .btn-gen-pass").click(function(b){b="";for(var a=0;32>a;a++)b+="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(62*Math.random()));document.getElementById(this.getAttribute("data-dst")).value=b;c(this).next(".fa-eye").attr("title",b)});c(".fa-eye").click(function(){var a=this.getAttribute("title");
""!==a&&alertify.alert(a)});c("#special_config_pass").on("keyup",function(){c(this).parent().find(".fa-eye").attr("title",this.value)});c("#btnBack").click(function(){location.href=m()});c(".container-state").on("click",function(a){a=document.getElementById(this.getAttribute("data-container"));"true"==a.getAttribute("aria-expanded")?(a.setAttribute("aria-expanded","false"),c(this).removeClass("fa-caret-up").addClass("fa-caret-down"),c(a).slideUp("slow")):(a.setAttribute("aria-expanded","true"),c(this).removeClass("fa-caret-down").addClass("fa-caret-up"),
c(a).slideDown("slow"))})},Config:e}}(jQuery);
